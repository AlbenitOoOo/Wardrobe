            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await _emailSender.SendEmailAsync(Input.Email, "Potwierdź swój email",
                        $"Potwierdź swoje konto do <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>klikając tutaj</a>.");

                    if (_userManager.Options.SignIn.RequireConfirmedAccount)
                    {
                        return RedirectToPage("Rejestracja Potwierdzenie", new { email = Input.Email, returnUrl = returnUrl });
                    }
                    else
                    {
